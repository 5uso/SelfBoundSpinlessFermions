...
if True:
    #for nfermions, (V0, sigma0) in it.product((2, 3, 4, 5), ((-19.063, 0.225), (-15.416, 0.3), (-13.262, 0.375))):
    for (nfermions, num_layers), (V0, sigma0) in it.product(((6, 3),(7, 3),(8, 3),(9, 3),), ((-13.262, 0.375),)):
        pt_save_every_ith=1000
        em_save_every_ith=1000#300
        ...
        dummy_bounds = (-10, 10)
        dummy_space = torch.linspace(dummy_bounds[0], dummy_bounds[1], n_mesh, requires_grad=True, device=device)
        avg_mesh = torch.cartesian_prod(*(dummy_space for _ in range(nfermions))).reshape(n_mesh**nfermions, nfermions)
        #"""
        ...
        loss_accum = []
        for epoch in range(start, postepochs+2):
            stats={}
            ...
            optim.zero_grad()
            loss.backward()  #populates leafs with grads
            torch.nn.utils.clip_grad_norm_(net.parameters(), 0.85)
            optim.step()
            ...
            if(epoch % em_save_every_ith == 0):
                torch.save({'epoch':epoch,
                            ...
                            'log_prob':sampler.log_prob.detach(),
                            'sigma':sampler.sigma},
                            model_path)# + f'f/epoch{epoch}.pt')
                writer.write_to_file(filename)# + f'f/epoch{epoch}.csv')
        ...
        with torch.no_grad():
            energy_var, energy_mean = torch.var_mean(elocal, unbiased=True)
            energy_std = (energy_var / (nwalkers * 4)).sqrt()

        output = f"{nfermions} fermions, V0: {V0:8.4f}, sigma0: {sigma0:6.4f} | Energy: {energy_mean:8.4f} +/- {energy_std:6.4f} (Epoch {epoch})\n"
        print("\n" + output)
        with open('results.txt', 'a') as f: f.write(output)
