...
class HarmonicOscillatorWithInteraction1D(nn.Module):
    ...
    def __init__(self, net: nn.Module, V0: float, sigma0: float, nchunks: int, trap_enabled: bool = True) -> None:
    ...
    def forward(self, x: Tensor) -> Tensor:
        _kin = self.kinetic(x)
        _pot = self.potential(x) if self.trap_enabled else torch.zeros_like(_kin)
...
class GaussianInteraction1D(HarmonicOscillatorWithInteraction1D):

    def __init__(self, net: nn.Module, V0: float, sigma0: float) -> None:
        super(GaussianInteraction1D, self).__init__(net, V0, sigma0, 0, False)
